...
from legent.scene_generation.room_spec import RoomSpec, LeafRoom
...
class ESHAController:
    """
    ESHAController orchestrates single-room scene generation and interactive control in LEGENT.
    ...
    def __init__(self, 
                 ...
                 scene_config = None) -> None:
        self.env = env or Environment(env_path="auto")
        ...
        self._scene_config = scene_config or SceneConfig(
            room_type="LivingRoom",
            dims=self._default_dims_for_room("LivingRoom"),
            include_other_items=False,
            items={"pumpkin": 1}
        )
        self._run_chat_loop: bool = False

    def generate_candidate(self) -> Dict[str, Any]:
        """
        Generate a single-room scene candidate using LEGENT's HouseGenerator.
        """
        room_spec = RoomSpec(
            room_spec_id="ESHA-SingleRoom",
            sampling_weight=1.0,
            spec=[LeafRoom(room_id=3, ratio=1, room_type=self._scene_config.room_type)],
        )

        # Split user items into receptacles (floor assets) and small objects
        recpt_counts, small_prefab_counts = self._split_items_into_receptacles_and_small(self._scene_config.items)

        generator = HouseGenerator(
            room_spec=room_spec,
            dims=self._scene_config.dims,
            objectDB=self.odb,
            unit_size=2.5,
        )

        scene = generator.generate(
            object_counts=small_prefab_counts,
            receptacle_object_counts=recpt_counts,
            room_num=1,
        )

        if not self._scene_config.include_other_items:
            scene = self._filter_scene_to_specified_only(scene, self._scene_config.items)

        return scene

    # ---------- Internals ----------

    def _filter_scene_to_specified_only(self, scene: Dict[str, Any], items: Optional[Dict[str, int]]) -> Dict[str, Any]:
        if not items:
            return scene
        ...
        def is_keep(inst: Dict[str, Any]) -> bool:
            ...
            prefab = inst["prefab"]
            pinfo = self.odb.PREFABS.get(prefab)
            if not pinfo:
                return True
            # Floor/wall tags
            ...
            otype = self.odb.OBJECT_TO_TYPE.get(prefab, "")
        ...
        if needed_parents:
            ...
            for p in parent_instances:
                ...
                if key not in existing_keys:
                    filtered.append(p)
        new_scene = self._deepcopy_scene(scene)
        new_scene["instances"] = filtered
        return new_scene

    def _is_small_object_instance(self, inst: Dict[str, Any]) -> bool:
    ...
    def _matches(self, inst: Dict[str, Any], name: str) -> bool:
        key = name.lower()
    ...
    def _scene_bbox(self, scene: Dict[str, Any]) -> Tuple[float, float, float, float]:
    ...
    def _place_small_objects_by_type_or_prefab(self, scene: Dict[str, Any], name: str, count: int) -> int:
        placer = RectPlacer(self._scene_bbox(scene))
        instances = list(scene["instances"])  # copy
    ...
    def _place_one_small_on_any_surface(
        self,
    ...
    def _place_floor_receptacles_by_type(self, scene: Dict[str, Any], type_key: str, count: int) -> Dict[str, Any]:
    ...
    def _recluster_smalls_near(self, scene: Dict[str, Any], target_xz: Tuple[float, float]) -> Dict[str, Any]:
        instances = []
        smalls = []
    ...
    def _cluster_by_category(self, scene: Dict[str, Any]) -> Dict[str, Any]:
        instances = []
    ...
    # def lock_scene(self, scene: Dict[str, Any]) -> None:
    #     """Lock-in a chosen scene for study and future modifications."""
    #     self._locked_scene = self._deepcopy_scene(scene)
    #     self.env.reset(ResetInfo(scene=self._locked_scene))
    ...
    #     """Generate a new scene using configured parameters and load it into the environment."""
    #     params = self._gen_params
    #     scenes = self.generate_candidates(
    #         room_type=params["room_type"],
    #         items=params["items"],
    #         include_other_items=params["include_other_items"],
    #         num_candidates=1,
    #         dims=params["dims"],
    #     )
    #     scene = scenes[0]
    #     self._current_scene = self._deepcopy_scene(scene)
    #     self.env.reset(ResetInfo(scene=self._current_scene))
    #     return scene

    # def run_chat_control(self, room_type: RoomType, items: Optional[Dict[str, int]] = None, include_other_items: bool = True, dims: Optional[Tuple[int, int]] = None) -> None:
    #     """
    #     Enter a simple loop that listens for chat commands from the in-game chat box:
    #       - "lock_scene()": lock the currently displayed scene
    #       - "reset_scene()": generate a new random layout (with configured params)
    #       - "quit()": exit the chat control loop
    #     """
    #     self.configure_generation(room_type, items, include_other_items, dims)
    #     if self._current_scene is None:
    #         self.reset_scene()
    #     self._run_chat_loop = True
    #     try:
    #         while self._run_chat_loop:
    #             obs = self.env.step()
    ...
    #                     self.lock_scene(self._current_scene)
    #             elif cmd == "reset_scene()":
    #                 self.reset_scene()
    #             elif cmd == "quit()":
    #                 self._run_chat_loop = False
    #             else:
    #                 # ignore other text; could be extended for more commands
    #                 pass
    #     finally:
    #         self._run_chat_loop = False
