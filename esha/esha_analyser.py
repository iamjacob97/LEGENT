"""
ESHA Analyzer: Environmental Survey Hazard Analysis for LEGENT scenes

This module provides an `ESHAAnalyzer` class that performs ESHA (Environmental
Survey Hazard Analysis) on scenes generated by LEGENT. It is designed to be a
pure wrapper on LEGENT core (no modifications to core code), consuming the
scene dictionary produced by LEGENT's scene generator and optionally one or
more scene screenshots. The output is a structured hazard analysis suitable for
downstream reporting and safety engineering workflows.

Key capabilities:
- Parse LEGENT scene dicts (instances, rooms, player/agent)
- Summarize environment features, objects, and agents per ESHA taxonomy
- Compute scene-level and room-level metrics (density, counts, areas)
- Optionally include images to augment LLM analysis
- Produce a comprehensive ESHA report using an LLM, with a rule-based fallback

Usage example:

    from legent.esha.esha_analyser import ESHAAnalyzer
    from legent.esha.esha_controller import ESHAController

    controller = ESHAController()
    scene = controller.generate_candidate()

    analyser = ESHAAnalyzer(api_key="...optional...", base_url=None)
    result = analyser.analyze(scene_dict=scene, images=["./topdown.png"])  # images optional

    print(result.report_markdown)
    # or consume structured JSON-like results
    print(result.summary)

The analyzer detects OpenAI at runtime if available. If the API key is not
provided or the client is unavailable, a safe rule-based fallback produces a
baseline ESHA report.
"""
...
class ESHAAnalyzer:
    """Performs ESHA on a LEGENT scene dict using rule-based analysis and LLMs.

    - Consumes the dict produced by `HouseGenerator.generate(...)` which
      includes keys such as `instances`, `room_polygon`, `player`, and `agent`.
    - Optionally augments the prompt with provided images.
    - Produces a structured report and a markdown narrative suitable for
      stakeholders.
    """
    ...
    def analyze(
        ...
        """Run ESHA analysis on the provided scene.

        Parameters:
            scene_dict: LEGENT scene dict from HouseGenerator.generate(...)
            images: Optional list of image file paths to include in the prompt
            extra_context: Optional freeform text to include for the LLM
        """
    ...
    def _parse_scene(self, scene: Dict[str, Any]) -> Dict[str, Any]:
        """Extracts structured scene information needed for ESHA.
...
def run_esha_analysis(
    scene_dict: Dict[str, Any],
    images: Optional[Sequence[str]] = None,
    api_key: Optional[str] = None,
    base_url: Optional[str] = None,
) -> ESHAResult:
    """One-shot ESHA analysis helper."""
    analyzer = ESHAAnalyzer(api_key=api_key, base_url=base_url)
